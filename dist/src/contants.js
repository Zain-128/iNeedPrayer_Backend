...
/** Hostinger Atlas integration often injects MONGODB_URI; we also accept MONGO_URI. */
export const MONGO_URI = process.env.MONGO_URI ||
    process.env.MONGODB_URI ||
    "mongodb://localhost:27017/iNeedPrayer";
export const JWT_SECRET = process.env.JWT_SECRET || "your-secret-key-change-in-production";
/** Access token TTL (used for login/register/social accessToken). */
export const JWT_EXPIRES_IN = process.env.JWT_EXPIRES_IN || "7d";
export const JWT_ACCESS_EXPIRES_IN = process.env.JWT_ACCESS_EXPIRES_IN || process.env.JWT_EXPIRES_IN || "7d";
...
export const JWT_REFRESH_SECRET = process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET || "your-secret-key-change-in-production";
...
export function publicUrlForUpload(relativePath) {
...
export const GOOGLE_TRANSLATE_API_KEY = (process.env.GOOGLE_TRANSLATE_API_KEY ?? "").trim();
/** Languages to auto-translate posts into (excluding detected source). */
export const TRANSLATION_TARGET_LANGUAGES = (process.env.TRANSLATION_TARGET_LANGUAGES ?? "es,fr,ar,hi,ur,pt")
    .split(",")
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean);
/** Agora RTC — required for live streaming token generation. */
export const AGORA_APP_ID = (process.env.AGORA_APP_ID ?? "").trim();
export const AGORA_APP_CERTIFICATE = (process.env.AGORA_APP_CERTIFICATE ?? "").trim();
export const AGORA_TOKEN_EXPIRY_SECONDS = Math.max(300, parseInt(process.env.AGORA_TOKEN_EXPIRY_SECONDS ?? "3600", 10) || 3600);
/** Host must heartbeat within this window or session auto-ends. */
export const LIVE_HOST_HEARTBEAT_TIMEOUT_MS = Math.max(30_000, parseInt(process.env.LIVE_HOST_HEARTBEAT_TIMEOUT_MS ?? "90000", 10) || 90_000);
