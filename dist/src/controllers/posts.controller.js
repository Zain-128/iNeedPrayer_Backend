...
export const pray = async (req, res) => {
    try {
        if (!req.userId)
            return res.status(401).json({ message: "Unauthorized" });
        const id = paramStr(req.params.id);
...
export const praise = async (req, res) => {
    try {
        if (!req.userId)
            return res.status(401).json({ message: "Unauthorized" });
        const id = paramStr(req.params.id);
...
export const prayPraiseUsers = async (req, res) => {
    try {
        const id = paramStr(req.params.id);
        if (!mongoose.isValidObjectId(id))
            return badId(res);
...
export const addComment = async (req, res) => {
    try {
        ...
        const comments = await commentsService.addComment(id, req.userId, text, typeof parentCommentId === "string" ? parentCommentId : undefined);
