...
import { JWT_SECRET, JWT_REFRESH_SECRET, JWT_ACCESS_EXPIRES_IN, JWT_REFRESH_EXPIRES_IN, PASSWORD_RESET_CODE, } from "../contants.js";
const SOCIAL_PROVIDERS = ["google", "apple", "facebook", "twitter"];
function signAccessToken(userId) {
    return jwt.sign({ userId, typ: "access" }, JWT_SECRET, { expiresIn: JWT_ACCESS_EXPIRES_IN });
}
function signRefreshToken(userId) {
    return jwt.sign({ userId, typ: "refresh" }, JWT_REFRESH_SECRET, { expiresIn: JWT_REFRESH_EXPIRES_IN });
}
...
export const socialLogin = async (input) => {
    ...
    if (!email || !name || !socialLoginId || !provider) {
        const err = new Error("Please provide email, name, socialLoginProvider, and socialLoginId");
        err.statusCode = 400;
        throw err;
    }
    if (!SOCIAL_PROVIDERS.includes(provider)) {
        const err = new Error(`socialLoginProvider must be one of: ${SOCIAL_PROVIDERS.join(", ")}`);
        err.statusCode = 400;
        throw err;
    }
    ...
    if (!user) {
        ...
        if (emailTaken) {
            const err = new Error("An account with this email already exists. Sign in with password or use the same social provider.");
            err.statusCode = 409;
            throw err;
        }
